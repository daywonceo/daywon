-- Create weekly_summaries and meal_plans tables with RLS, triggers, and indexes
...
-- Policies: users can manage their own summaries
CREATE POLICY IF NOT EXISTS "users_select_own_weekly_summaries"
ON public.weekly_summaries
FOR SELECT TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "users_insert_own_weekly_summaries"
ON public.weekly_summaries
FOR INSERT TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "users_update_own_weekly_summaries"
ON public.weekly_summaries
FOR UPDATE TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "users_delete_own_weekly_summaries"
ON public.weekly_summaries
...
CREATE POLICY IF NOT EXISTS "users_select_own_meal_plans"
ON public.meal_plans
FOR SELECT TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "users_insert_own_meal_plans"
ON public.meal_plans
FOR INSERT TO authenticated
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "users_update_own_meal_plans"
ON public.meal_plans
FOR UPDATE TO authenticated
USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "users_delete_own_meal_plans"
ON public.meal_plans
FOR DELETE TO authenticated
USING (auth.uid() = user_id);
